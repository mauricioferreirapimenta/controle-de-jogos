...
COLUNAS = [
    "Plataforma", "Console", "Gênero", "Jogo", "Mídia", "Edição", "Condição",
    "Status", "Nota", "Tempo (h)", "Início", "Fim", "Observações coleção",
    "Comentários pessoais", "Data de lançamento"
]
    ...
    if df.empty:
        ...
        if plataforma:
            ...
            with st.form("editar"):
                ...
                data_lancamento = st.date_input("Data de lançamento", value=tratar_data(df.at[idx, "Data de lançamento"]))
                ...
                inicio = st.date_input("Início", value=tratar_data(df.at[idx, "Início"]))
                fim = st.date_input("Fim", value=tratar_data(df.at[idx, "Fim"]))
...
if pagina == "📁 Backup e Importação":
    ...
    st.download_button("📥 Exportar para Excel", buffer.getvalue(), "backup_jogos.xlsx", mime="application/vnd.ms-excel")
