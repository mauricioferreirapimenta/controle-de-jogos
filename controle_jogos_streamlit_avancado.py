...
# Estrutura das colunas completas
colunas = [
    "Plataforma", "Console", "Gênero", "Jogo", "Mídia", "Edição",
    "Condição", "Status", "Nota", "Tempo (h)",
    "Início", "Fim", "Observações coleção", "Comentários pessoais"
]
...
# --- Seção 1: Adicionar novo jogo à coleção ---
...
with st.form("adicionar_jogo"):
    ...
    with col1:
        ...
        condicao = st.text_input("Condição / Observações de coleção (ex: Lacrado, Completo, etc.)")

    with col2:
        console = st.text_input("Console")
        edicao = st.selectbox("Edição", edicao_opcoes)
        jogo = st.text_input("Nome do Jogo")
        obs_colecao = st.text_area("Observações gerais da mídia (manual, caixa, etc.)")
...
# --- Seção 2: Editar detalhes de gameplay ---
st.subheader("🛠️ Editar detalhes de gameplay")
if not df.empty:
...
if not df.empty:
    colunas_ordenadas = colunas  # manter ordem manual
