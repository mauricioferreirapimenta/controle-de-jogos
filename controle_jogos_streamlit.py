...
# Plataforma e console selecionados FORA do formulário para permitir comportamento dinâmico
...
# Seção para deletar jogo
st.subheader("🗑️ Excluir jogo da lista")
if not df.empty:
    jogo_para_excluir = st.selectbox("Selecione o jogo para deletar", df["Jogo"])
    if st.button("Excluir jogo"):
        df = df[df["Jogo"] != jogo_para_excluir]
        df.to_excel(ARQUIVO, index=False)
        st.success(f"🗑️ '{jogo_para_excluir}' foi removido com sucesso!")
