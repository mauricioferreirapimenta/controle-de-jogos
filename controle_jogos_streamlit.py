...
console_edit = st.selectbox("Console", [""] + sorted(df[df["Plataforma"] == plat_edit]["Console"].dropna().unique()), key="edit_console") if plat_edit else ""
jogos_edit = df[(df["Plataforma"] == plat_edit) & (df["Console"] == console_edit)]["Jogo"].unique() if console_edit else []
...
console_excluir = st.selectbox("Console", [""] + sorted(df[df["Plataforma"] == plat_excluir]["Console"].dropna().unique()), key="excluir_console") if plat_excluir else ""
jogos_excluir = df[(df["Plataforma"] == plat_excluir) & (df["Console"] == console_excluir)]["Jogo"].unique() if console_excluir else []
jogo_excluir = st.selectbox("Jogo", [""] + list(jogos_excluir), key="excluir_jogo")

if st.button("Excluir jogo selecionado") and jogo_excluir:
...
col1, col2 = st.columns(2)
with col1:
    st.download_button("⬇️ Exportar para Excel", df.to_excel(index=False, engine='openpyxl'), file_name="backup_colecao_jogos.xlsx")
with col2:
